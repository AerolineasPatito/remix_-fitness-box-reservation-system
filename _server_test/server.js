...
async function startServer() {
    ...
    `);
        ...
        const classTypesCount = db.prepare(`SELECT COUNT(*) as count FROM class_types`).get();
      ...
      `);
            defaults.forEach((row) => insertType.run(createId('ctype_'), row.name, row.image_url, row.icon, row.color_theme, row.description, row.duration));
        }
        const ensureSystemSetting = (key, value) => {
    ...
    `).run();
        const whatsappTemplatesCount = db.prepare(`SELECT COUNT(*) as count FROM whatsapp_templates`).get();
      ...
      `).run(createId('watpl_'), 'Cancelación de clase', 'Hola {{nombre_alumno}}, tu clase "{{clase_activa}}" del {{fecha_cancelacion}} fue cancelada. Si necesitas apoyo para reagendar, estoy pendiente.', 'system');
        }
      ...
      `).get(key);
        ...
        const sanitizeHourMinute = (raw, fallback) => {
        ...
        const getCancellationCutoffMorning = () => sanitizeHourMinute(getSettingValue('cancellation_cutoff_morning', '08:00'), '08:00');
        const getCancellationDeadlineEvening = () => sanitizeHourMinute(getSettingValue('cancellation_deadline_evening', '22:00'), '22:00');
        const buildDateWithClock = (baseDate, hhmm) => {
            const [hour, minute] = hhmm.split(':').map(Number);
            const next = new Date(baseDate);
            next.setHours(hour, minute, 0, 0);
            return next;
        };
        const calculateCancellationDeadline = (classDate, classStartTime) => {
        ...
        const formatCancellationDeadlineLabel = (deadline) => deadline.toLocaleString('es-MX', {
            weekday: 'long',
            day: '2-digit',
            month: 'long',
            year: 'numeric',
        ...
        const formatDateTimeLabel = (date, startTime, endTime) => {
    ...
    `;
        ...
        `).get(params.classId);
                ...
                if (classRow.status !== 'active') {
                    return { classRow, reservations: [], businessEmail: getBusinessNotificationEmail() };
                }
        ...
        const ensureTitularBeneficiary = (subscriptionId) => {
      ...
      `).run(createId('ben_'), subscriptionId, subscription.alumno_id, Number(subscription.clases_totales || 0), Number(subscription.clases_restantes || 0), subscription.estado === 'active' ? 'active' : subscription.estado);
        };
        const syncStudentCredits = (studentId) => {
      ...
      `).run(Number(balance.total || 0), studentId);
        ...
        const redistributeBeneficiaries = (subscriptionId) => {
            const subscription = db.prepare(`
      ...
      `).all();
        ...
        if (!existingAdmin) {
            ...
            db.prepare('INSERT INTO profiles (id, email, full_name, password_hash, role, credits_remaining) VALUES (?, ?, ?, ?, ?, ?)').run('admin-1', adminEmail, 'Admin Focus', hash, 'admin', 999);
    ...
    `).run();
      ...
      `).get();
        ...
        const sanitizeUserFacingError = (message, statusCode) => {
            if (!message) {
      ...
      `).all();
          ...
          `).get(cls.id);
                    ...
                    if (Number(activeReservations.count || 0) < minRequired) {
                        ...
                        for (const reservation of result.reservations) {
                            ...
                            const studentSent = await sendEmailSafe(reservation.email, `Clase cancelada - ${result.classRow.type}`, studentHtml);
                            db.prepare(`
                UPDATE reservations
        ...
        app.post('/api/auth/register', async (req, res) => {
            ...
            try {
                ...
                db.prepare('INSERT INTO profiles (id, email, full_name, password_hash, role, credits_remaining, whatsapp_phone, email_verified, email_verification_token, email_verification_expires) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(id, email, fullName || 'Atleta Focus', hash, role, 10, normalizedWhatsapp, 0, verificationToken, verificationExpires.toISOString());
      ...
      `).get(reservationId);
                if (reservationDetails?.email && reservationDetails?.class_name && reservationDetails?.class_date) {
                    ...
                    const cancellationDeadline = calculateCancellationDeadline(reservationDetails.class_date, String(reservationDetails.start_time || '').slice(0, 5));
      ...
      `).run(id, nombre, parsedCapacity, parsedTotalClasses, Number(vigencia_semanas), detalles || '', Number(precio_base || 0), estado || 'active', actor_id || null, actor_id || null);
        ...
        WHERE id = ?
          AND deleted_at IS NULL
      `).run(nombre, parsedCapacity, parsedTotalClasses, Number(vigencia_semanas), detalles || '', Number(precio_base || 0), estado || 'active', actor_id || null, req.params.id);
      ...
      `).run(req.params.id);
        ...
        app.post('/api/coach/subscriptions', (req, res) => {
            try {
                refreshSubscriptions();
                const { alumno_id, paquete_id, fecha_compra, metodo_pago, referencia, monto, actor_id, notas } = req.body;
      ...
      `).get(paquete_id);
        ...
        `).run(subscriptionId, alumno_id, paquete_id, purchaseDate.toISOString(), expiryDate.toISOString(), totalClasses, totalClasses, notas || null);
                    db.prepare(`
          INSERT INTO suscripcion_beneficiarios (
            id, suscripcion_id, alumno_id, es_titular, clases_asignadas, clases_restantes, estado, created_at, updated_at
          ) VALUES (?, ?, ?, 1, ?, ?, 'active', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
        `).run(createId('ben_'), subscriptionId, alumno_id, totalClasses, totalClasses);
        ...
        `).run(paymentId, subscriptionId, alumno_id, paquete_id, amountToRegister, metodo_pago || 'manual', referencia || null, purchaseDate.toISOString());
      ...
      `).all();
        ...
        WHERE id = ?
          AND role = 'student'
          AND deleted_at IS NULL
      `).run(full_name ?? null, email_verified == null ? null : (email_verified ? 1 : 0), patient_external_id ?? null, req.params.id);
        ...
        `).get(studentId);
          ...
          `).run(subscriptionId, studentId, purchaseDate.toISOString(), expiryDate.toISOString(), delta, delta, reason || 'Créditos manuales');
                        db.prepare(`
            INSERT INTO suscripcion_beneficiarios (
              id, suscripcion_id, alumno_id, es_titular, clases_asignadas, clases_restantes, estado, created_at, updated_at
            ) VALUES (?, ?, ?, 1, ?, ?, 'active', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
        ...
        `).run(adjustmentId, studentId, targetSubscriptionId, actor_id || null, delta, reason || null, beforeBalance, afterBalance);
      ...
      `).all(req.params.id);
          ...
          `).all(...subscriptionIds)
                    : [];
                const beneficiariesMap = beneficiariesBySubscription.reduce((acc, row) => {
                    if (!acc[row.suscripcion_id])
                        acc[row.suscripcion_id] = [];
      ...
      `).all(req.params.id, req.params.id);
        ...
        WHERE deleted_at IS NULL
        ORDER BY year DESC
      `).all().map((row) => row.year).filter((y) => !!y);
        ...
        ORDER BY month ASC
      `).all(String(selectedYear)).map((row) => row.month).filter((m) => !!m);
                const baseFilter = `
      ...
      `).get(class_type_id);
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(id, classType.name, classType.id, date, start_time, end_time, maxCapacity, minCapacity, maxCapacity, created_by);
      ...
      `).run(id, String(name).trim(), String(body).trim(), Number(is_default_cancellation || 0) === 1 ? 1 : 0, actor_id || null, actor_id || null);
        ...
        SET name = COALESCE(?, name),
            ...
            updated_at = CURRENT_TIMESTAMP
        WHERE id = ?
      `).run(name ? String(name).trim() : null, body ? String(body).trim() : null, is_default_cancellation == null ? null : (Number(is_default_cancellation) === 1 ? 1 : 0), actor_id || null, req.params.id);
      ...
      `).run(req.params.id);
        ...
        app.post('/api/classes/recurring', (req, res) => {
            try {
                const { class_type_id, start_date, start_time, end_time, min_capacity, max_capacity, created_by, recurrence_type, weeks, days_of_week } = req.body || {};
      ...
      `).get(class_type_id);
                ...
                const customDays = Array.isArray(days_of_week)
                    ? days_of_week.map((d) => Number(d)).filter((d) => Number.isInteger(d) && d >= 0 && d <= 6)
                    : [];
      ...
      `);
                ...
                const tx = db.transaction(() => {
                    dateList.forEach((classDate) => {
                        ...
                        insert.run(classId, classType.name, classType.id, classDate, start_time, end_time, maxCapacity, minCapacity, maxCapacity, created_by || null);
        ...
        `).get(reservationId);
                    ...
                    const cancellationDeadline = calculateCancellationDeadline(reservation.date, String(reservation.start_time || '00:00').slice(0, 5));
                    const now = new Date();
        ...
        `).get(reservationId);
                ...
                if (details) {
                    ...
                    businessSent = await sendEmailSafe(businessEmail, `Cancelación de reserva - ${details.class_name}`, buildBusinessCancellationEmail({
                    ...
                    studentSent = await sendEmailSafe(details.email, `Confirmación de cancelación - ${details.class_name}`, buildStudentCancellationEmail({
        ...
        app.post('/api/class-types', (req, res) => {
            try {
                ...
                if (!name)
                    return res.status(400).json({ error: 'El nombre del tipo de entrenamiento es obligatorio.' });
                const id = createId('ctype_');
                db.prepare(`
        INSERT INTO class_types (id, name, image_url, icon, color_theme, description, duration, is_active, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
      `).run(id, name, image_url || null, icon || null, color_theme || null, description || null, Number(duration || 60), is_active === 0 ? 0 : 1);
        ...
        SET name = COALESCE(?, name),
            ...
            updated_at = CURRENT_TIMESTAMP
        WHERE id = ?
      `).run(name || null, image_url || null, icon || null, color_theme || null, description || null, duration != null ? Number(duration) : null, is_active, req.params.id);
      ...
      `).get(userId, userId);
        ...
        app.get('/api/admin/stats', (req, res) => {
            ...
            const totalCredits = db.prepare('SELECT SUM(credits_remaining) as total FROM profiles').get();
            res.json({
    ...
    `).all();
        ...
        app.post('/api/admin/profiles', async (req, res) => {
            ...
            try {
                ...
                db.prepare('INSERT INTO profiles (id, email, full_name, password_hash, role, credits_remaining) VALUES (?, ?, ?, ?, ?, ?)').run(id, email, full_name, hash, role, credits_remaining || 0);
                res.json({ success: true });
            }
            catch (error) {
                res.status(500).json({ error: error.message });
